...
//管理员工号检测
 function checkadminname(){
        ...
        adminid = encodeURIComponent(adminid);//特殊字符处理
        if(adminid==''){			
        ...
        //ajax抓取到工号传递给服务器        
        ajaxObj.open('GET','/huaxian/public/index.php/admin/Admin/check?adminid=' + adminid);
        ajaxObj.send(null);
    }
...
function checkusername(){
        ...
        username = encodeURIComponent(username);//特殊字符处理
        var ajaxObj = new XMLHttpRequest();
        ajaxObj.onreadystatechange = function(){
            if(ajaxObj.readyState==4){
                document.getElementById('usercheck').innerHTML = ajaxObj.responseText;
            }
        }       
        //ajax抓取到用户名传递给服务器        
        ajaxObj.open('GET','/huaxian/public/index.php/admin/User/usernamecheck?username=' + username);
        ajaxObj.send(null);
    }
...
function checkimg(){
        ...
        code = encodeURIComponent(code);//特殊字符处理
        var ajaxObj = new XMLHttpRequest();
        ajaxObj.onreadystatechange = function(){
            if(ajaxObj.readyState==4){
                document.getElementById('imgcheck').innerHTML = ajaxObj.responseText;
            }
        }       
        //ajax抓取到用户名传递给服务器        
        ajaxObj.open('GET','/huaxian/public/index.php/admin/Login/checkimg?code=' + code);
        ajaxObj.send(null);
    }
